...
Release 0.9.2 changes the visibility of the `PdfiumLibraryBindingsAccessor` trait from `pub(crate)` to `pub`, easing use of the raw `FPDF_*` API by crate consumers thanks to an excellent observation by <https://github.com/martin-kolarik>, and changes the signature of the `PdfBitmap::empty()` function to remove the `bindings` argument, which is no longer required.
...
* 0.9.2 changes the visibility of the `PdfiumLibraryBindingsAccessor` trait from `pub(crate)` to `pub`, easing use of the raw `FPDF_*` API by crate consumers, thanks to an excellent observation by <https://github.com/martin-kolarik>; changes the signature of the `PdfBitmap::empty()` function to remove the `bindings` argument, which is no longer required.
