...
* 0.8.37: adds new `PdfRenderConfig::set_fixed_size()`, `PdfRenderConfig::set_fixed_width()`, and `PdfRenderConfig::set_fixed_height()` functions for more finely-grained rendering control thanks to an excellent suggestion by <https://github.com/newinnovations>; adds new `PdfRenderConfig::set_fixed_size_to_bitmap()` and `PdfRenderConfig::scale_page_to_bitmap()` functions; adds new `PdfPageImageObject::get_raw_image_data()` function thanks to an excellent contribution from <https://github.com/zamf>; adds new functions `PdfPageObject::is_active()`, `PdfPageObject::set_active()`, `PdfPageObject::is_inactive()`, and `PdfPageObject::set_inactive()` for setting and retrieving the object status on its containing page.
