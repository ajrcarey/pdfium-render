...
Release 0.8.37 adds the new `PdfPageImageObject::get_raw_image_data()` function thanks to an excellent contribution from <https://github.com/zamf>, and adds new functions `PdfPageObject::is_active()`, `PdfPageObject::set_active()`, `PdfPageObject::is_inactive()`, and `PdfPageObject::set_inactive()` for setting and retrieving the object status on its containing page. Page objects marked as inactive are retained in the document, but are not included in page rendering.
